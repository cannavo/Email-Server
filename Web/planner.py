...
def AddTask():
    ...
    if ('submit' in request.form):
        ...
        listdate=request.form['completeOn']
        printY=listdate.split('-',1)[0]
        printM=listdate.split('-',2)[1]
        printD=listdate.split('-',1)[1]
        printD=printD.split('-',1)[1]
        listdate=str(printM)+"-"+str(printD)+"-"+str(printY)
...
#        print("past decsription")
        ...
        newTask=Task(str(task),str(duedate),str(description),str(category),str(idNum),str(username),str(listdate),str(False),str(priority)) 
        dao.insert(newTask)
#        print("DONE")
        return redirect(url_for('TaskList'))
